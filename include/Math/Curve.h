...
public:
	...
	{
		if ( !_pathPts ) {
			_pathPts = new osg::Vec3Array;
		}
		_pathPts->push_back( v );
	}

	/** Specifies a vertex list as the curve path. */
	inline void setPath( osg::Vec3Array* pts )
	{
		_pathPts=pts;
	}
	inline osg::Vec3Array* getPath()
	{
		return _pathPts.get();
	}
	inline const osg::Vec3Array* getPath() const
	{
		return _pathPts.get();
	}
	...
	{
		return _pathPts->back()==_pathPts->front();
	}
