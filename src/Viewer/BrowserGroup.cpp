...
void BrowserGroup::setSelectedNodes( QLinkedList<osg::ref_ptr<Data::Node> >* selected )
{
	...
	for ( i = selected->begin(); i != selected->end(); ++i ) {
		...
		if ( !selectedNodes->contains( node ) && Lua::LuaGraph::getInstance()->getNodes()->contains( node->getId() ) ) {
			...
			Lua::LuaTypes::LuaValueMap paramsTable = luaNode->getParams().asTable();
			...
			if ( !browsersGrouping ) {
				Lua::LuaTypes::LuaValueMap models;
				models.insert( std::pair<Lua::LuaTypes::LuaValue, Lua::LuaTypes::LuaValue>( ( long )node->getId(), luaNode->getParams() ) );
...
void BrowserGroup::addBrowser( const std::string& templateType, osg::Vec3 position, Lua::LuaTypes::LuaValueMap models )
{
