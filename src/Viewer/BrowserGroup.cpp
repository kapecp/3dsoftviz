...
void BrowserGroup::setSelectedNodes( QLinkedList<osg::ref_ptr<Data::Node> >* selected )
{
	...
	Data::Graph* activeGraph = Manager::GraphManager::getInstance()->getActiveGraph();
	...
	for ( i = selected->begin(); i != selected->end(); ++i ) {
		...
		if ( !selectedNodes->contains( node ) && Lua::LuaGraph::getInstance()->getNodes()->contains( node->getId() ) ) {
			...
			// Special case for Moonscript graph - add browser even if no metrics table found
			if ( activeGraph->getName() == "MoonscriptGraph" ) {
				QString luaNodeId = QString::fromStdString( luaNode->getParams().getValue()["origId"].asString() );
				this->addBrowser( "moonscript", node->getCurrentPosition(), luaNodeId );
				continue;
			}
...
void BrowserGroup::addBrowser( const std::string& templateType, osg::Vec3 position, QString luaNodeId )
{
	// qDebug() << "Adding browser";
...
//	connectorTransform->setUserValue( "frame", 0 );
	...
	// Display template in webview
	webView->showMoonscriptTemplate( "class_template", templateType, luaNodeId );
}
