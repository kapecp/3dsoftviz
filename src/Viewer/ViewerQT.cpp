...
osg::Node* QOSG::ViewerQT::createModel() {
    ...
    osg::Node* loadedModel = this->cg->getScene();//createBase();
    ...
    if (!loadedModelTransform) return 0;
    ...
    // then create the camera node to do the render to texture
    {
        ...
        // set up projection.
        //camera->setProjectionMatrixAsPerspective( 60, static_cast<double>( width() )/static_cast<double>( height() ), 0.01, 100/*appConf->getValue( "Viewer.Display.ViewDistance" ).toFloat()*/ );
        ...
        camera->addChild(loadedModelTransform);
    }
    cg->setCamera(camera);
    ...
    osg::Vec3 projectorPos = osg::Vec3(0.0f, -2.0f, 0.5f);
    osg::Vec3 projectorDirection = osg::Vec3(0.0f, 2.0f, -1.0f);
