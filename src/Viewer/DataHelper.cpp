...
osg::ref_ptr<osg::Vec3Array> DataHelper::getInitialVectors( int count )
{
	...
	for ( int index = 0; index < count; index++ ) {
		double x, y, z;
		x = lowest + ( range * static_cast<double>( random() ) / ( RAND_MAX + 1.0 ) );
		y = lowest + ( range * static_cast<double>( random() ) / ( RAND_MAX + 1.0 ) );
		z = lowest + ( range * static_cast<double>( random() ) / ( RAND_MAX + 1.0 ) );
...
int DataHelper::getRandomNumber( int lowest, int highest )
{
	...
	return lowest + static_cast<int>( range * random() / ( RAND_MAX + 1.0 ) );
}
