...
osg::Vec3 Curve::mapTo( const osg::Vec3 p, osg::BoundingBox originRect, osg::BoundingBox newRect )
{
	...
	if ( originSpace.y() ) {
		scaleFactor.y() /= originSpace.y();
	}
	else {
		scaleFactor.y() = 0.0;
	}
	if ( originSpace.z() ) {
		scaleFactor.z() /= originSpace.z();
	}
	else {
		scaleFactor.z() = 0.0;
	}
...
osg::Vec2 Curve::mapTo2D( const osg::Vec3 p, osg::BoundingBox originRect, osg::BoundingBox newRect )
{
	if ( originRect.xMin()==originRect.xMax() ) {
		newRect.zMin() = newRect.yMin();
		newRect.zMax() = newRect.yMax();
		newRect.yMin() = newRect.xMin();
		newRect.yMax() = newRect.xMax();
		newRect.xMin() = 0.0;
		newRect.xMax() = 0.0;
	}
	...
	if ( originRect.xMin()==originRect.xMax() ) {
		point2D.set( newPoint.y(), newPoint.z() );
	}
	else if ( originRect.yMin()==originRect.yMax() ) {
		point2D.set( newPoint.x(), newPoint.z() );
	}
