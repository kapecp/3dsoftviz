...
osgModeling::BezierCurve::BezierCurve():
	osgModeling::Curve(),
	_method( 1 ), _ctrlPts( 0 ), _cont( 0.0f ), _degree( 3 ), _numPath( 20 )
{
}

osgModeling::BezierCurve::BezierCurve( const BezierCurve& copy, const osg::CopyOp& copyop/*=osg::CopyOp::SHALLOW_COPY*/ ):
...
osgModeling::BezierCurve::BezierCurve( osg::Vec3Array* pts, unsigned int degree, unsigned int numPath ):
	osgModeling::Curve(),
...
osgModeling::BezierCurve::BezierCurve( unsigned int stride, unsigned int order, double* ptr, unsigned int numPath ):
	osgModeling::Curve(),
	...
	for ( unsigned int i=0; i<size; i+=stride ) {
		if ( stride==2 ) {
			_ctrlPts->push_back( osg::Vec3(
									 ( float )*( ptr+i ), ( float )*( ptr+i+1 ), 0.0f ) );
		}
		else {
			_ctrlPts->push_back( osg::Vec3(
...
osg::Vec3 osgModeling::BezierCurve::lerpRecursion( osg::Vec3Array* pts, unsigned int r, unsigned int i, double u )
{
	if ( r==0 ) {
		return ( *pts )[i];
	}
...
double osgModeling::BezierCurve::bernstein( int k, int i, double u )
{
	double basis = factorial( k ) / ( factorial( i ) * factorial( k-i ) );
	basis *= ( u==0.0 && i==0 ) ? 1.0f : pow( u, i );
	basis *= ( u==1.0 && i==k ) ? 1.0f : pow( 1-u, k-i );
	return basis;
}

void osgModeling::BezierCurve::updateImplementation()
{
	if ( !_ctrlPts ) {
		return;
	}
	...
	if ( _method==0 ) {
		useBernstein( pathArray.get() );
	}
	else if ( _method==1 ) {
		useDeCasteljau( pathArray.get() );
	}
	setPath( pathArray.get() );
}

void osgModeling::BezierCurve::useBernstein( osg::Vec3Array* result )
{
...
void osgModeling::BezierCurve::useDeCasteljau( osg::Vec3Array* result )
{
