...
void AdapterWidget::keyPressEvent( QKeyEvent* event )
{
	...
	switch ( event->key() ) {
		...
		default: {
			_gw->getEventQueue()->keyPress( static_cast<osgGA::GUIEventAdapter::KeySymbol>( *( event->text().toLatin1().data() ) ) );
			break;
		}
	}
}
...
void AdapterWidget::keyReleaseEvent( QKeyEvent* event )
{
	...
	switch ( event->key() ) {
		...
		default: {
			_gw->getEventQueue()->keyRelease( static_cast<osgGA::GUIEventAdapter::KeySymbol>( *( event->text().toLatin1().data() ) ) );
			break;
		}
	}
}
