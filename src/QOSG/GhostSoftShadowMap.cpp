...
//
static const char fragmentCustomSoftShaderSource_noBaseTexture[] =
...
// fragment shader
//
static const char fragmentCustomSoftShaderSource_withBaseTexture[] =
	"#define SAMPLECOUNT 64 \n"
	"#define SAMPLECOUNT_FLOAT 64.0 \n"
	"#define SAMPLECOUNT_D2 32 \n"
	"#define SAMPLECOUNT_D2_FLOAT 32.0 \n"
...
GhostSoftShadowMap::GhostSoftShadowMap():
...
GhostSoftShadowMap::GhostSoftShadowMap( const GhostSoftShadowMap& copy, const osg::CopyOp& copyop ):
...
void GhostSoftShadowMap::setJitteringScale( float jitteringScale )
{
	_jitteringScale = jitteringScale;
	if ( _jitteringScaleUniform.valid() ) {
		_jitteringScaleUniform->set( _jitteringScale );
	}
}

void GhostSoftShadowMap::setSoftnessWidth( float softnessWidth )
{
	_softnessWidth = softnessWidth;
	if ( _softnessWidthUniform.valid() ) {
		_softnessWidthUniform->set( _softnessWidth );
	}
}

void GhostSoftShadowMap::setJitterTextureUnit( unsigned int jitterTextureUnit )
{
	_jitterTextureUnit=jitterTextureUnit;
}

void GhostSoftShadowMap::createUniforms()
{
	_uniformList.clear();
...
void GhostSoftShadowMap::createShaders()
{
	// if we are not given shaders, use the default
	if ( _shaderList.empty() ) {
		if ( _shadowTextureUnit==0 ) {
			osg::Shader* fragment_shader = new osg::Shader( osg::Shader::FRAGMENT, fragmentCustomSoftShaderSource_noBaseTexture );
			_shaderList.push_back( fragment_shader );
		}
		else {
			osg::Shader* fragment_shader = new osg::Shader( osg::Shader::FRAGMENT, fragmentCustomSoftShaderSource_withBaseTexture );
			_shaderList.push_back( fragment_shader );
...
void GhostSoftShadowMap::initJittering( osg::StateSet* ss )
{
