...
int igraph_layout_fruchterman_reingold_3d(const igraph_t *graph, 
					  igraph_matrix_t *res,
					  igraph_integer_t nstep, igraph_real_t maxdelta,
					  igraph_real_t volume, igraph_real_t coolexp,
					  igraph_real_t repulserad,
					  igraph_bool_t use_seed, igraph_integer_t niter_of_step,
					  igraph_integer_t step_index,
					  const igraph_vector_t *weight) {
  ...
  for(i=(niter_of_step*(nstep-step_index));i>=(niter_of_step*(nstep-step_index-1));i--){
   /* if (i%10 == 0)
      IGRAPH_PROGRESS("3D Fruchterman-Reingold layout: ",
		      100.0-100.0*i/niter, NULL);*/

    /*Set the temperature (maximum move/iteration)*/
    t=maxdelta*pow(i/(double)(niter_of_step*nstep),coolexp);
    /*Clear the deltas*/
